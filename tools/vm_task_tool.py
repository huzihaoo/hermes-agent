...
def _build_scheduler_meta(
    *,
    ...
    if codex_backend_enabled is not None and not (
        values["executor_type"] == "coding_agent" and values["agent_backend"] == "codex"
    ):
        return {}, "invalid codex_backend_enabled: only allowed for coding_agent with codex backend"
...
def _spawn_completion_probe_background(task_id: str) -> dict[str, Any]:
    ...
    if not task_id or not _TASK_ID_RE.match(task_id):
        return {"started": False, "reason": "missing_or_invalid_task_id"}
