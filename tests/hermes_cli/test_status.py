...
def test_show_status_reports_local_mem0_sidecar_summary(monkeypatch, capsys, tmp_path):
    from hermes_cli import status as status_mod
    import hermes_cli.auth as auth_mod
    import hermes_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(
        status_mod,
        "load_config",
        lambda: {"model": "gpt-5.4", "memory": {"provider": "local_mem0_sidecar"}},
        raising=False,
    )
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "openai-codex", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "openai-codex", raising=False)
    ...
    output = capsys.readouterr().out
    assert "Memory:      local_mem0_sidecar (configured)" in output
    assert "Memory URL:  http://127.0.0.1:8765" in output
    assert "Memory Policy: search-only, candidate-gated, reviewed-auto-recall" in output
