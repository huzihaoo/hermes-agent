...
def test_vm_task_submit_rejects_codex_gate_on_non_codex_execution_plane(monkeypatch, tmp_path):
    _disable_trusted_session(monkeypatch)
    script = tmp_path / "create_task_v2.py"
    script.write_text("print('unused')", encoding="utf-8")
    monkeypatch.setattr(vm_task_tool, "_create_task_script", lambda: script)
    ...
    def fake_run(*args, **kwargs):
        nonlocal called
        called = True
        raise AssertionError("subprocess should not run for invalid Codex gate metadata")
    ...
    assert result["success"] is False
    assert "invalid codex_backend_enabled" in result["error"]
    assert called is False
