...
def test_bootstrap_prepare_is_read_only_then_steady_only_validates_and_transitions(
    ...
    gate_projection = release_gate._check_capacity_runtime_projection(
        runtime_decision,
        config={
            "capacity_mode": "bootstrap",
            "release_id": CAPACITY_RELEASE_ID,
        ...
        artifact="bootstrap_pre_production_gate",
    )
    assert gate_projection["effective_state"] == (
        capacity_transition.BOOTSTRAP_PRODUCTION
    )
    assert gate_projection["sample_count"] == 0
