...
class AIAgent:
            ...
            try:
                with open(os.devnull, "w") as _devnull, \
                    ...
                    review_agent = AIAgent(
                        ...
                        provider=self.provider,
                        api_mode=_parent_runtime.get("api_mode") or self.api_mode,
                        base_url=_parent_runtime.get("base_url") or self.base_url,
                        api_key=_parent_runtime.get("api_key") or self.api_key,
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            if _stream_read_timeout_raw is None and self.base_url and is_local_endpoint(self.base_url):
                _stream_read_timeout = max(_stream_read_timeout, _base_timeout)
                logger.debug(
                    "Local provider detected (%s) — stream read timeout raised to %.0fs",
                    self.base_url, _stream_read_timeout,
                )
            elif _stream_read_timeout_raw is None and self._is_remote_longrun_endpoint():
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if not _execution_blocked and not self.tool_progress_callback and not self.tool_start_callback and _tool_emitter is not None:
                try:
                    _tool_emitter.emit("tool:call", {
                        "task_id": effective_task_id,
                        "tool_name": function_name,
