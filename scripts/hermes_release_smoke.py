...
def collect_smoke(args: argparse.Namespace) -> dict[str, Any]:
    ...
    high_signal_lines = [
        line
        for line in _tail_lines(gateway_error_log, args.log_tail_lines)
        if any(pattern in line for pattern in HIGH_SIGNAL_LOG_PATTERNS)
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Hermes host release smoke harness")
