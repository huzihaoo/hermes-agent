...
Kafka SASL principal 由 owner 提供，必须以字面量 `rca_` 开头，并且只允许 ASCII 字母、数字、下划线、点和连字符。principal、固定 consumer Group `rca_root_cause_analysis_agent` 与固定 client/service identity `root_cause_analysis_agent` 是三个不同概念，不得强制相等。
