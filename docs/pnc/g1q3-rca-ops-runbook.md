...
Host formal gate 必须同时消费两份证据，缺一即硬阻断：VM 生成的 `remote_reader_soak.json`，以及 data owner 的**原始 canonical 输入字节**按原样复制到 Host evidence 目录后的 `remote_reader_soak_manifest.json`。复制过程不得 parse/re-serialize；Host 目标必须是 owner-only `0600`、regular、same-euid、single-link、no symlink，父 evidence 目录不得 group/world-writable。发布 receipt 同时记录两个 hash：
